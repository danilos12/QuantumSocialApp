	...
	if(isset( $r['wp_user_id'] ) ) {
		...
		if( !is_numeric(base64_decode($r['wp_user_id']))  ) {
            ...
            if(User::where('email',$checkExistingEmail)->exists()){
				$laravel_id = User::where('email',$checkExistingEmail)->value('id');
                return response()->json(['status' =>'error', 'laravel_id' => $laravel_id]);
            }
				if(base64_decode($r['wp_product_id']) != '839'){
			...
			return response()->json(['received?'=>'hety','status' =>'success', 'laravel_id' => $user->id]);
