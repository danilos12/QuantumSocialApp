...
// LightMode Settings
var n_stars = 120;
var colors = ["#741A67", "#43EBF1", "#F400C4", "#72FF8F"];
for (let i = 0; i < 98; i++) {
...
$newSlot.click(function (e) {
    ...
    if ($newSlotOverlay.css("display") == "block") {
        ...
        $(".new-slot-form select")
            .find('option[value="' + day + '"]')
            .attr("selected", true);
        $(".new-slot-form .new-slot-time-wrapper select#hour-selector")
            .find('option[value="' + row + '"]')
            .attr("selected", true);
        $(".new-slot-form .new-slot-time-wrapper select#minute-selector")
            .find('option[value="00"]')
            .attr("selected", true);
        $(".new-slot-form .new-slot-time-wrapper select#am-pm-selector")
            .find('option[value="' + ampm + '"]')
            .attr("selected", true);
    }
});
